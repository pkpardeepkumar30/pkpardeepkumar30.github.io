...
| Web applications page | `src/data/projects.json` |
| Publications | `src/data/publications.json` |
| News on the home page | `src/data/news.json` |
| Research text | `src/pages/research.md` |
| Software experience | `src/pages/software.md` |
...
Edit `src/data/projects.json`. Each entry controls one card on the dedicated Web Applications page at `/web/`. Use an empty `live` value when a project does not yet have a public deployment; the live-site link will be hidden automatically. Keep `source` pointed at the public repository.

The Web page includes a download link for the web-focused résumé and keeps product work distinct from scientific-software experience.
...
## Update the career pages, résumés, and portrait

- Edit the software profile in `src/pages/software.md`, web profile in `src/pages/web.astro`, and educator profile in `src/pages/teaching.md`.
- The three PDFs are `public/cv/Resume_Pardeep_Kumar_Software.pdf`, `public/cv/Resume_Pardeep_Kumar_Web.pdf`, and `public/cv/Resume_Pardeep_Kumar_Teaching.pdf`.
- All three are generated by `scripts/generate_resume.py`; run `python scripts/generate_resume.py` after editing that script. It requires the Python package `reportlab`.
- Replace `public/images/profile.jpg` with a new portrait using the same filename when needed.

## Add ORCID and GitHub

The ORCID entry already exists in `src/site.config.ts` with an empty URL. Add the real URL and it will appear automatically on the home page. Other profiles can be added by copying one item in the same `social` list.
